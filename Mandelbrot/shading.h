...
// TODO: restructure to be more generic
// std::vector<MultiPassShader> phases;

struct ShaderPassSurfaces
{
    deferred_unique_ptr<ShaderSurface> pass_9x9;
    deferred_unique_ptr<ShaderSurface> pass_3x3;
    deferred_unique_ptr<ShaderSurface> pass_1x1;

    std::vector<deferred_unique_ptr<ShaderSurface>> phase_surfaces;

    ShaderPassSurfaces(int phase_count, DeferredDestructionQueue& q)
        : pass_9x9(make_deferred_unique<ShaderSurface>(q))
        , pass_3x3(make_deferred_unique<ShaderSurface>(q))
        , pass_1x1(make_deferred_unique<ShaderSurface>(q))
    {
        phase_surfaces.reserve((size_t)phase_count);
        for (int i = 0; i < phase_count; ++i)
            phase_surfaces.emplace_back(make_deferred_unique<ShaderSurface>(q));
    }

    ShaderPassSurfaces(ShaderPassSurfaces&&) noexcept = default;
    ShaderPassSurfaces& operator=(ShaderPassSurfaces&&) noexcept = default;

    ShaderPassSurfaces(const ShaderPassSurfaces&) = delete;
    ShaderPassSurfaces& operator=(const ShaderPassSurfaces&) = delete;

    void setFragmentSource(std::string_view src)
    {
        for (auto& s : phase_surfaces)
            s->setFragmentSource(src);
    }

    const ShaderSurface* getSurface(int phase) const
    {
        return phase_surfaces[(size_t)phase].get();
    }

    ShaderSurface* getSurface(int phase)
    {
        return phase_surfaces[(size_t)phase].get();
    }
...
static inline void createShaderPasses(std::string_view script, std::vector<ShaderPassSurfaces> &pass_surfaces, int phase_count=3)
{
    struct PassMeta
    {
        bool enabled = true;
    ...
    // shrink (no default construction required)
    while (pass_surfaces.size() > passes.size())
        pass_surfaces.pop_back();

    // grow (construct with required args)
    pass_surfaces.reserve(passes.size());
    while (pass_surfaces.size() < passes.size())
        pass_surfaces.emplace_back(phase_count, main_window()->deferredGuiDestructionQueue());
