...
ImU32 TextEditor::GetGlyphColor(const Glyph& aGlyph) const
{
	...
	// allow directive tokens to override comment coloring
	if (aGlyph.mColorIndex == PaletteIndex::PreprocIdentifier)
		return mPalette[(int)PaletteIndex::PreprocIdentifier];
...
void TextEditor::Render()
{
	...
	if (!mLines.empty())
	{
		...
		bool calltip_shown = false;
		if (ImGui::IsWindowFocused())
		{
			std::string decl;
			Coordinates name_start;
			if (GetSignatureHelpAtCursor(decl, name_start))
			{
				ImVec2 lineStartScreenPos = ImVec2(cursorScreenPos.x, cursorScreenPos.y + name_start.mLine * mCharAdvance.y);
				ImVec2 textScreenPos = ImVec2(lineStartScreenPos.x + mTextStart, lineStartScreenPos.y);
				ImVec2 tipPos = ImVec2(textScreenPos.x + TextDistanceToLineStart(name_start), lineStartScreenPos.y + mCharAdvance.y + 4.0f);

				ImGui::SetNextWindowPos(tipPos);
				ImGui::BeginTooltip();
				ImGui::TextUnformatted(decl.c_str());
				ImGui::EndTooltip();
				calltip_shown = true;
			}
		}
		...
		if (!calltip_shown && ImGui::IsMousePosValid())
		{
...
bool TextEditor::IsIdentifierChar(Char c)
{
	return c == '_' || std::isalnum(static_cast<unsigned char>(c)) != 0;
}

bool TextEditor::StepLeft(int& ioLine, int& ioIndex) const
{
	// ioIndex is a byte index in [0..line.size()], stepping left lands on a UTF-8 lead byte
	while (true)
	{
		if (ioLine < 0 || ioLine >= (int)mLines.size())
			return false;
		...
		if (ioIndex == 0)
		{
			...
			--ioLine;
			ioIndex = (int)mLines[ioLine].size();
			continue;
		}

		--ioIndex;

		while (ioIndex > 0 && IsUTFSequence((char)mLines[ioLine][ioIndex].mChar))
			--ioIndex;
...
bool TextEditor::GetSignatureHelpAtCursor(std::string& outDeclaration, Coordinates& outNameStart) const
{
	outDeclaration.clear();
	//outNameStart = Coordinates::Invalid();
	outNameStart = Coordinates(-1, -1);

	// avoid showing calltips while selecting
	if (HasSelection())
		return false;

	const Coordinates cursor = GetActualCursorCoordinates();
	if (cursor.mLine < 0 || cursor.mLine >= (int)mLines.size())
		return false;

	int scanLine = cursor.mLine;
	int scanIndex = GetCharacterIndex(cursor);
	if (scanIndex < 0)
		return false;
	...
	while (StepLeft(scanLine, scanIndex))
	{
		const Char c = mLines[scanLine][scanIndex].mChar;
		...
		if (c == '(')
		{
			if (parenDepth == 0)
			{
				openParenLine = scanLine;
				openParenIndex = scanIndex;
				break;
			}
	...
	// Find identifier directly before '(' (same line only).
	int nameLine = openParenLine;
	int nameIndex = openParenIndex;

	// Skip spaces/tabs between name and '('.
	while (true)
	{
		int prevLine = nameLine;
		int prevIndex = nameIndex;
		if (!StepLeft(prevLine, prevIndex))
			return false;

		if (prevLine != nameLine)
			return false;

		const Char c = mLines[prevLine][prevIndex].mChar;
		if (c != ' ' && c != '	')
		{
			nameIndex = prevIndex;
			break;
	...
	if (!IsIdentifierChar(mLines[nameLine][nameIndex].mChar))
		return false;

	int nameStartIndex = nameIndex;
	while (nameStartIndex > 0)
	{
		int prevIndex = nameStartIndex - 1;
		while (prevIndex > 0 && IsUTFSequence((char)mLines[nameLine][prevIndex].mChar))
			--prevIndex;

		if (!IsIdentifierChar(mLines[nameLine][prevIndex].mChar))
			break;

		nameStartIndex = prevIndex;
	}

	const int nameEndIndex = nameIndex + UTF8CharLength(mLines[nameLine][nameIndex].mChar);
	const Coordinates nameStart(nameLine, GetCharacterColumn(nameLine, nameStartIndex));
	const Coordinates nameEnd(nameLine, GetCharacterColumn(nameLine, nameEndIndex));
	const std::string name = GetText(nameStart, nameEnd);

	auto it = mLanguageDefinition.mIdentifiers.find(name);
	if (it == mLanguageDefinition.mIdentifiers.end() || it->second.mDeclaration.empty())
		return false;

	outDeclaration = it->second.mDeclaration;
